    ...
    if request.method == 'POST':
        ...
        if user is not None:
            try:
                ...
                is_car_item_exists = CartItem.objects.filter(cart=cart).exists()
...
def logout(request):
    auth.logout(request)
    messages.success(request, 'has salido de seccion')
    return redirect('login')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            send_email = EmailMessage(mail_subject, body, to=[to_email])
            send_email.send()
            messages.success(request, 'fue enviado un email a '+email+' para resetear tu password')
            return redirect('login')
        else:
...
def resetPassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Por favor resetea el password')
